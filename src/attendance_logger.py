...
class AttendanceLogger:
    ...
    def _in_cooldown(self, conn, employee: str, event_type: str, now: datetime) -> bool:
        """Check if the same event type is within cooldown period.

        Different event types (checkin vs checkout) have independent cooldowns,
        allowing a quick checkin followed by checkout when needed.
        """
        row = conn.execute(
